...
import { addCustomerSchema, type AddCustomerInput } from "../../../../shared/schemas/customer.schema";
import { useAddCustomer } from "../../hooks/customersHooks/useAddCustomer";
import Loader from "../ui/Loader";

const InputCustomerModal = ({ onClose }: { onClose: () => void }) => {
    const { register, handleSubmit, formState: { errors } } = useForm<AddCustomerInput>({
        resolver: zodResolver(addCustomerSchema),
    });

    const { mutate: addCustomer, isPending, error } = useAddCustomer();

    const onSubmit = (data: AddCustomerInput) => {
        addCustomer(data, { onSuccess: onClose });
    };
        ...
        >
            ...
            >
                ...
                <p className="mt-2 text-sm text-tertiary/60">
                    Completá los datos del nuevo cliente frecuente.
                </p>
